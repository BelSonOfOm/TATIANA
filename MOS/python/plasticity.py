...
class CriticalityMonitor:
    ...
    def __init__(self, default_epsilon_rho: float = 0.10, min_history: int = 20,
                 quantile: float = 0.75, window: int = 500):
        self.default_epsilon_rho = default_epsilon_rho
    ...
    def epsilon_rho(self) -> float:
        ...
        if len(self.rho_history) < self.min_history:
            return self.default_epsilon_rho
...
if __name__ == "__main__":
    ...
    m = CriticalityMonitor(default_epsilon_rho=0.10, min_history=20)
    assert m.epsilon_rho() == 0.10
    print("    epsilon() with no history =", m.epsilon_rho(), " (falls back, no guessing)")
