...
def _simulate_mixture_margin(mix: Mixture, T: int, rng: np.random.Generator,
                             row_sizes: np.ndarray,
                             mask: Optional[np.ndarray] = None) -> np.ndarray:
    """As `_simulate_mixture`, but every simulated tick has a row sum drawn from
    the OBSERVED row-size distribution.

    WHY THIS IS NECESSARY, NOT COSMETIC. Under top-k retrieval every observed row
    has EXACTLY k ones. `_simulate_mixture` emits independent Bernoullis, whose
    row sums are Binomial and therefore have variance that the data does not.
    A null that differs from the data in row-size distribution is testing that
    difference as well as the structure -- so the noisy-OR/mixture margin would
    be contaminated by a feature of the RETRIEVAL RULE rather than of the latent
    structure. Matching the margin removes that confound.

    HONEST STATEMENT OF WHAT THIS SAMPLER IS. Gumbel-top-k gives *successive*
    weighted sampling without replacement (equivalently Plackett-Luce): draw
    without replacement with probability proportional to the component's Bernoulli
    parameters. That is NOT the exact maximum-entropy conditional-Bernoulli
    distribution (Fisher's noncentral hypergeometric) with fixed row sum -- the
    two agree in support and are close in marginals, but they are different
    distributions. This is an approximation and is labelled as one. The column
    margins it produces are checked rather than assumed; see
    `test_margin_null_recovers_column_margins`.
    """
    N = mix.p.shape[0]
    comp = rng.choice(mix.K, size=T, p=mix.w / mix.w.sum())
    W = mix.p[:, comp].T                                   # (T, N) emission probs
    if mask is not None:
        W = W * mask
    ks = rng.choice(np.asarray(row_sizes), size=T).astype(int)

    # Gumbel-top-k: argmax over log w + Gumbel noise, taken k at a time, is
    # exactly sampling without replacement with probability proportional to w.
    keys = np.log(np.maximum(W, 1e-300)) + rng.gumbel(size=(T, N))
    keys[W <= 0] = -np.inf                    # a concept that cannot fire, cannot

    X = np.zeros((T, N), dtype=float)
    uniq = np.unique(ks)
    if uniq.size == 1:                        # the top-k case: one partition, fast
        k = int(min(uniq[0], N))
        if k > 0:
            idx = np.argpartition(-keys, k - 1, axis=1)[:, :k]
            np.put_along_axis(X, idx, 1.0, axis=1)
        return X
    order = np.argsort(-keys, axis=1)         # variable-width case
    for t in range(T):
        k = int(min(ks[t], N))
        if k > 0:
            X[t, order[t, :k]] = 1.0
...
def curveball_randomize(X: np.ndarray, n_swaps: Optional[int] = None,
                        rng: Optional[np.random.Generator] = None) -> np.ndarray:
    """Randomise a binary matrix holding BOTH row and column sums exactly fixed.

    The 'curveball' / Strona et al. trade (Nature Communications 2014): pick two
    rows, keep what they share, and randomly redeal what they do not. Row sums are
    preserved because each row gets back as many items as it gave; column sums are
    preserved because every dealt item stays in exactly one of the two rows.

    WHY THIS EXISTS AND WHAT IT IS FOR -- read this before using it, because it is
    NOT a drop-in replacement for `_simulate_mixture_margin`:

      * `_simulate_mixture_margin` answers "is a COVER a better explanation than a
        PARTITION?" -- its null is data generated by a fitted partition.
      * `curveball_randomize` answers a DIFFERENT and more basic question: "is
        there ANY co-occurrence structure beyond what the margins already force?"
        Its null is the maximum-entropy ensemble over binary matrices with the
        observed margins -- which is exactly Jaynes's construction, the same
        max-entropy principle that gives the Boltzmann distribution.

    Ecology has fought over this class of null since Connor & Simberloff (1979);
    Gotelli's systematic test found the fixed-row-sum models to be the ones that
    do not inflate Type I error. See DOCS/PRIOR_ART_AND_THE_REPLAN.md 1.

    ⚠️ STILL OPEN, STATED RATHER THAN PAPERED OVER. This does NOT fix the sampling
    bias in `_simulate_mixture_margin`. That sampler needs draws from the fitted
    mixture CONDITIONAL on a fixed row sum -- the conditional-Bernoulli law --
    whereas Gumbel-top-k gives successive sampling. The measured deviation is a
    column-margin correlation of 0.985 (test_cover_mask). Curveball cannot repair
    it because curveball knows nothing about the mixture. Exact conditional
    Bernoulli sampling remains owed.
    """
    rng = rng or np.random.default_rng(0)
    rows = [set(np.flatnonzero(r).tolist()) for r in np.asarray(X)]
    T = len(rows)
    if T < 2:
        return np.array(X, dtype=float, copy=True)
    n_swaps = int(n_swaps if n_swaps is not None else 5 * T)

    for _ in range(n_swaps):
        i, j = int(rng.integers(T)), int(rng.integers(T))
        if i == j:
            continue
        A, B = rows[i], rows[j]
        shared = A & B
        ...
        if not only_a or not only_b:
            continue                       # nothing tradeable; the pair is inert
        pool = only_a + only_b
        rng.shuffle(pool)
        na = len(only_a)
        rows[i] = shared | set(pool[:na])
        rows[j] = shared | set(pool[na:])
...
def calibrated_compare(X: np.ndarray, K: int, n_folds: int = 5, B: int = 49,
                       ...
                       match_margins: bool = False) -> CalibratedComparison:
    ...
    `match_margins` -- REQUIRED whenever retrieval imposes a row-size constraint,
    which top-k retrieval does exactly (every assembly has |A| = k). The default
    False preserves the historical behaviour bit-for-bit so existing tests and
    prior results are unchanged; see `_simulate_mixture_margin` for why the
    unmatched null is invalid under top-k and for what the sampler is and is not.
    """
    folds = block_split(X.shape[0], n_folds)
    if not folds:
        raise ValueError("Not enough ticks to form held-out blocks.")
