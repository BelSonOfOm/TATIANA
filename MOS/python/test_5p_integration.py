...
print("=" * 68)
mon = CriticalityMonitor(default_epsilon_rho=0.10, min_history=20)
print(f"  cold start: epsilon = {mon.epsilon_rho():.3f} (documented default, no guess)")
for _ in range(300):
    mon.record_rho(float(abs(rng.normal(0.05, 0.02))))
print(f"  after 300 rho observations: epsilon = {mon.epsilon_rho():.4f} "
